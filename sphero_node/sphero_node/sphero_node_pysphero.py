...
#from geometry_msgs.msg import Point, Pose,MultiThreadedExecutor Quaternion, Twist, Vector3
...
#from sphero_node.transformations.transformations import quaternion_from_euler
...
#            pixel = sphero_user_io.Pixel(x=3, y=4)
    ...
    def sub_cmd_vel(self, msg):
        ...
        if self.is_connected:
            self.last_cmd_vel_time = datetime.now()
            #Compute headinMultiThreadedExecutorg given the velocities. Atan2 is called as atan2(y,x) but here we have to change the coordinate system
            ...
            if self.cmd_speed >= 0:
                ...
                self.robot.driving.drive_with_heading(self.cmd_speed, self.cmd_heading, Direction.reverse)
    ...
    def pub_odom(self):
        ...
        tmp_data = copy.deepcopy(sensor_data)
...
def notify_callback(data: Dict):
    ...
    info = ", ".join("{:1.2f}".format(data.get(param)) for param in ps_sensor.Velocity)
    print(f"[{data.get(CoreTime.core_time):1.2f}] Velocity (x, y): {info}")
    info = ", ".join("{:1.2f}".format(data.get(param)) for param in ps_sensor.Gyroscope)
    print(f"[{data.get(CoreTime.core_time):1.2f}] Velocity (x, y): {info}")
